  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8000/api/recent")
      ...
      .then((events: DashboardEvent[]) => {
        const tweetEvents = events.filter(
          (e) => e.type === "tweet_received"
        ) as TweetEvent[];
          ...
          <div className="max-h-96 overflow-y-auto">
            {trades.length === 0 ? (
              ...
              trades.map((trade, index) => (
                <div
                  key={`${trade.data.market_name}-${index}`}
                  className="p-4 border-b border-gray-100 last:border-b-0 hover:bg-gray-50"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
